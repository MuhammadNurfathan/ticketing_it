<x-app-layout>
    ...
    <div class="min-h-screen bg-light-bg dark:bg-dark-bg">
        <div class="w-full px-4 sm:px-6 lg:px-8 py-6 space-y-6">
            ...
            <div id="previewModal"
                ...
                <div class="bg-white dark:bg-dark-eval-1 rounded-2xl shadow-2xl w-11/12 max-w-6xl p-6 transform scale-95 transition-all">
                    ...
                    <div class="overflow-x-auto max-h-[60vh] border border-gray-200 dark:border-gray-700 rounded-lg">
                        <table class="min-w-full text-sm">
                            <thead class="bg-gray-50 dark:bg-dark-eval-2 sticky top-0">
                                <tr>
                                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Ticket Code</th>
                                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Requestor</th>
                                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Support</th>
                                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Problem</th>
                                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Status</th>
                                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Start</th>
                                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">End</th>
                                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Time (min)</th>
                                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Late</th>
                                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Created</th>
                                </tr>
                            </thead>
                            <tbody id="previewTableBody" class="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-dark-eval-1">
                                </tbody>
                        </table>
                    </div>
                    ...
                    <div class="mt-6 flex justify-end gap-3">
                        <button id="closePreview"
                            class="px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-dark-eval-2 transition-colors text-sm font-medium"> Tutup
                        </button>
                ...
                <div class="p-4 sm:p-6">
                    ...
                    <div class="relative flex justify-center items-center min-h-[400px]">
                        <div id="barLoadingIndicator"
                            ...
                            <p class="mt-4 text-gray-600 dark:text-gray-400">Memuat data...</p> </div>
                        ...
                        <div class="space-y-2">
                            ${
                                tickets.length
                                    ...
                                    `).join('')
                                    : '<p class="text-center text-gray-400 dark:text-gray-500 italic">Tidak ada tiket</p>' // Sudah ok
                            }
                        </div>
                    </div>
                `;
    ...
    {{-- Bar Charts Script --}}
    <script>
    // Satu warna netral yang cukup terlihat di latar belakang gelap dan terang
    ...
    // --- DIJADIKAN SATU WARNA NEUTRAL ---
    function getTextColor() {
        return NEUTRAL_COLOR; // Teks selalu abu-abu netral
    }
    // --- DIJADIKAN SATU WARNA NEUTRAL ---
    function getGridColor() {
        return NEUTRAL_COLOR; // Garis grid selalu abu-abu netral
    }

    // Warna background card (tetap dinamis agar border bar terlihat)
    function getBgColor() {
        return isDarkMode() ? '#1f2937' : '#ffffff'; // DARK = GELAP, LIGHT = PUTIH
    }
    ...
    // Load Bar Chart - Tickets per Dev
    function loadBarChart(year) {
        ...
        fetch(`/api/chart-tickets-by-dev?year=${year}`)
            .then(res => res.json())
            .then(res => {
                    ...
                    barChart = new Chart(canvas, {
                        ...
                        data: {
                            ...
                            datasets: res.data.datasets.map((dataset, idx) => ({
                                ...dataset,
                                backgroundColor: getBarColors()[idx % getBarColors().length],
                                // MENGGUNAKAN getBgColor() AGAR BORDER BAR KONSISTEN DENGAN BACKGROUND CARD
    ...
    // Load Time Chart - Time Spent per Dev
    function loadTimeChart(year) {
        ...
        fetch(`/api/chart-time-spent-by-dev?year=${year}`)
            .then(res => res.json())
            .then(res => {
                loading.classList.add('hidden');
    ...
    function getBarChartOptions() {
        // Panggil helper functions untuk memastikan options selalu up-to-date
        const textColor = getTextColor(); // Sudah NEUTRAL_COLOR
        const gridColor = getGridColor(); // Sudah NEUTRAL_COLOR
        ...
        return {
            ...
            plugins: {
                legend: {
                    ...
                    position: 'top',
                    labels: {
                        color: textColor, // Menggunakan NEUTRAL_COLOR
                        font: { size: 12 },
                        padding: 15
                    }
                },
                tooltip: {
                    // TOOLTIP BACKGROUND: TETAP DINAMIS AGAR KONTRAST DENGAN TEXT
                    backgroundColor: isDarkMode() ? '#374151' : '#ffffff', 
                    titleColor: textColor, // Menggunakan NEUTRAL_COLOR
                    bodyColor: textColor, // Menggunakan NEUTRAL_COLOR
                    borderColor: gridColor, // Menggunakan NEUTRAL_COLOR
                    borderWidth: 1
                }
            },
            scales: {
                x: {
                    grid: {
                        color: gridColor, // Menggunakan NEUTRAL_COLOR
                        drawBorder: false
                    },
                    ticks: {
                        color: textColor, // Menggunakan NEUTRAL_COLOR
